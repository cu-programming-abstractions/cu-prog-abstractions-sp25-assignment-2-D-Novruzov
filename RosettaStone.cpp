...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if(kGramLength <= 0) {
        error("kgram should be positive");
    }
    ...
    for (int i = 0; i < str.length(); i++) {
        ...
        if(temp.length() == kGramLength) {
            if(newMap.containsKey(temp)) {
                newMap[temp]++;
            }
            else {newMap.put(temp, 1);}
            temp = temp.substr(1);
        }
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    double sum = 0.0;
    bool allzero = true;
    Vector<double> values = newMap.values();
    Vector<string> keys = newMap.keys();
    for(double value : values) {
        if (value != 0) {
            allzero = false;
        }}
    if(allzero|| newMap.isEmpty()) {error("there is sth with a map");}
    for(const string& key: keys){
        sum += pow(newMap[key], 2.0);
    }

    for(const string&  key : keys) {
            newMap[key] = newMap[key] / sqrt(sum);
        }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    if(numToKeep < 0){error("numtokeep shoudl be negative");}
    for(const string& key :  keys) {
