...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    for(int i = 0; i < sources.size(); i++){
        if(terrain.get(sources[i].row, sources[i].col) <= height) {
            flooded.set(sources[i], true);
            q.enqueue(sources[i]);
        }
        else{flooded.set(sources[i], false);}

    }
    while(!q.isEmpty()) {
        GridLocation curr =  q.dequeue();
        Vector<GridLocation> neighbours = {   {curr.row - 1, curr.col},
                                            {curr.row + 1, curr.col},
                                            {curr.row, curr.col - 1},
                                            {curr.row, curr.col + 1}
        };
        for(const GridLocation& neighbour : neighbours) {
            if(terrain.inBounds(neighbour.row, neighbour.col)){
                if(terrain.get(neighbour.row, neighbour.col) <= height && !flooded.get(neighbour.row, neighbour.col)){
                    flooded.set(neighbour.row, neighbour.col, true);
                    q.enqueue(neighbour);
                }
            }
        }
